    ...
    public function index(Request $request)
    {
        ...
        ///
           $contacts =  Contact :: where(function($query) use ($request) {
                            ...
                            $query->where("contacts.user_id", $request->user()->id);
                            ...
                            {
                                $keywords = '%' . $term . '%';
                                $query->where("contacts.name", 'LIKE', $keywords);
                                $query->orWhere("contacts.company", 'LIKE', $keywords);
                                $query->orWhere("contacts.email", 'LIKE', $keywords);
                            }
//                            if ($status_id = ($request->get('status_id'))) {
//                                $query->whereIn('contacts.status_id', $status_id);
//                            }
                        })
                        ->orderBy('id', 'desc')
                      ->paginate($this->limit);
...
//        echo '<pre>';
//        var_dump($request->user()->contacts()->create($data));
//        echo '</pre>';
//        die();
